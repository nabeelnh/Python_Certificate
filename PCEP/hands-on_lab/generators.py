...
def char_range(start, stop, step=1):
    stop_modifier = 1
    start_code = ord(start)                                         # convert to unicode code points each character
    stop_code = ord(stop)                                           # convert to unicode code points each character
    
    if start_code > stop_code:                                      # Iterate backwards if the start code point is higher than the stop code point
        step *= -1                                                  # stepping backwords
        stop_modifier *= -1

    for each_value in range(start_code, stop_code + stop_modifier, step):
        yield chr(each_value)                                       # convert from int (unicode code points) to string
